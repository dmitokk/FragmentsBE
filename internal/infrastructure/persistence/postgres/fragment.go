...
func (r *FragmentRepository) Create(ctx context.Context, fragment *entity.Fragment) error {
	query := `
		INSERT INTO fragments (id, user_id, text, geomark, sound_url, photo_urls, expires_at, created_at, updated_at)
	...
	err := r.db.QueryRowContext(ctx, query,
		...
		time.Now(),
		time.Now(),
	).Scan(&fragment.ID, &fragment.ExpiresAt, &fragment.CreatedAt, &fragment.UpdatedAt)
...
func (r *FragmentRepository) GetByID(ctx context.Context, id uuid.UUID) (*entity.Fragment, error) {
	query := `
		SELECT id, user_id, text, ST_X(geomark) as lng, ST_Y(geomark) as lat, 
		       sound_url, photo_urls, expires_at, created_at, updated_at
		FROM fragments
		WHERE id = $1
	`
...
func (r *FragmentRepository) List(ctx context.Context, userID uuid.UUID, lat, lng, radius float64) ([]*entity.Fragment, error) {
	query := `
		SELECT id, user_id, text, ST_X(geomark) as lng, ST_Y(geomark) as lat, 
		       sound_url, photo_urls, expires_at, created_at, updated_at
		FROM fragments
		WHERE ST_DWithin(geomark::geography, ST_MakePoint($1, $2)::geography, $3)
